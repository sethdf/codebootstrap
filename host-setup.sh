...
cb() {
    local container=$(docker ps -qf "label=devcontainer.local_folder=$HOME/codebootstrap")
    if [ -z "$container" ]; then
        ...
        container=$(docker ps -qf "label=devcontainer.local_folder=$HOME/codebootstrap")
    fi
...
cb-status() {
    ...
    if [ -n "$container" ]; then
        ...
        docker ps -f "label=devcontainer.local_folder=$HOME/codebootstrap" --format "  ID: {{.ID}}\n  Created: {{.RunningFor}}\n  Status: {{.Status}}"
    else
